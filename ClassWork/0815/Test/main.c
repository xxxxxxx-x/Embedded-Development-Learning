...
int main() {
  ...
  // 2.加载字库
  font* f = fontLoad("/usr/share/fonts-droid-fallback/truetype/DroidSansFallback.ttf");

  // 3.设置字体的大小
  fontSetSize(f, 64);

  // 4.创建一块输出框
  bitmap* bm = createBitmap(64 * 3, 100, 4);
  // getColor(A, B, G, R)  设置颜色
  // 5.把汉字放入输出框
  fontPrint(f, bm, 0, 0, "大家好", getColor(0, 0, 255, 0), 0);
  show_font_to_lcd((void*)lcd, 400, 0, bm);
}