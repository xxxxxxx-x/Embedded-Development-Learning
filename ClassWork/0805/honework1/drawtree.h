...
static _linkqueue init_queue(void) {
  ...
  q->front = q->rear = (struct _queue_node *)malloc(sizeof(struct _queue_node));
  q->rear->next = NULL;

  return q;
}

static bool is_empty_q(_linkqueue q) { return (q->front == q->rear); }
...
#ifdef QUEUE_SIZE
int queue_size(_linkqueue *q) { return q->size; }
#endif
/* ↓↓↓↓↓↓↓↓↓↓↓↓↓ 画网页相关算法代码 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ */
...
static void data_leftside(int fd, TREENODE *root, int spaces) {
  ...
  if (root->lchild != NULL) {
    s_line = get_index(root->data) - get_index(root->lchild->data) - 1;
  }
...
static int data_rightside(int fd, TREENODE *root) {
  ...
  if (root->rchild != NULL) {
    s_line = get_index(root->rchild->data) - get_index(root->data) - 1;
  }
...
static void start_page(int fd) { write(fd, page_begin, strlen(page_begin)); }

/* ↓↓↓↓↓↓↓↓↓↓↓↓↓ 画网页相关算法代码 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ */
static void end_page(int fd) { write(fd, page_end, strlen(page_end)); }
...
static void draw(TREENODE *root) {
  ...
  snprintf(filename, FILENAME, "%s.html", "BSTreeNode");
