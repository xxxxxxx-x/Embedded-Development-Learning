...
uint8_t fixed_ip_text_data[19][128] = {
    /* "当" 的点阵数据 */
...
uint8_t fixed_time_text_data[19][128] = {
    /* "当" 的点阵数据 */
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
     0x00, 0x01, 0xC0, 0x00, 0x00, 0x01, 0x80, 0x00, 0x02, 0x01, 0x81, 0xC0,
     0x01, 0x81, 0x81, 0xC0, 0x01, 0xC1, 0x83, 0x00, 0x00, 0xE1, 0x86, 0x00,
    ...
    /* "时" 的点阵数据 */
    {
        ...
        0x00, 0x00, 0x00, 0x30, 0x3F, 0xF8, 0x00, 0x30, 0x3F, 0xF8, 0x00, 0x30,
        0x30, 0x18, 0x00, 0x30, 0x30, 0x1B, 0xFF, 0xFF, 0x30, 0x1B, 0xFF, 0xFF,
        0x30, 0x18, 0x00, 0x30, 0x30, 0x18, 0x00, 0x30, 0x30, 0x18, 0x40, 0x30,
        0x3F, 0xF8, 0xE0, 0x30, 0x3F, 0xF8, 0x60, 0x30, 0x30, 0x18, 0x70, 0x30,
        0x30, 0x18, 0x30, 0x30, 0x30, 0x18, 0x18, 0x30, 0x30, 0x18, 0x10, 0x30,
        0x30, 0x18, 0x00, 0x30, 0x30, 0x18, 0x00, 0x30, 0x3F, 0xF8, 0x00, 0x30,
        0x3F, 0xF8, 0x00, 0x30, 0x30, 0x18, 0x00, 0x30, 0x30, 0x18, 0x3F, 0xF0,
        0x00, 0x00, 0x3F, 0xE0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /*"时",0*/
        /* (32 X 32 , 微软雅黑 )*/
    },
    /* "间" 的点阵数据 */
    {
        ...
        0x0F, 0x00, 0x00, 0x00, 0x07, 0x8F, 0xFF, 0xFE, 0x01, 0xCF, 0xFF, 0xFE,
        0x00, 0x80, 0x00, 0x06, 0x30, 0x00, 0x00, 0x06, 0x30, 0x00, 0x00, 0x06,
        0x30, 0x3F, 0xFF, 0x06, 0x30, 0x3F, 0xFF, 0x06, 0x30, 0x30, 0x03, 0x06,
        0x30, 0x30, 0x03, 0x06, 0x30, 0x30, 0x03, 0x06, 0x30, 0x3F, 0xFF, 0x06,
        0x30, 0x3F, 0xFF, 0x06, 0x30, 0x30, 0x03, 0x06, 0x30, 0x30, 0x03, 0x06,
        0x30, 0x30, 0x03, 0x06, 0x30, 0x3F, 0xFF, 0x06, 0x30, 0x3F, 0xFF, 0x06,
        0x30, 0x30, 0x00, 0x06, 0x30, 0x00, 0x00, 0x06, 0x30, 0x00, 0x00, 0x06,
        0x30, 0x00, 0x03, 0xFE, 0x30, 0x00, 0x03, 0xFC, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /*"间",1*/
        /* (32 X 32 , 微软雅黑 )*/
    },
    /* "为" 的点阵数据 */
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
...
uint8_t *get_char_data_16x32(char c) {
  ...
  case ':': {
    uint8_t temp[64] = {
        ...
        0x03, 0xC0, 0x01, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x80, 0x03, 0xC0,
        0x03, 0xC0, 0x01, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /*":",0*/
    };
...
// 显示固定文本在 LCD 上
void display_fixed_ip_text_on_lcd(uint32_t (*lcd)[800], int x_offset,
                                  int y_offset, int scale) {
  int x = x_offset;
  ...
  for (int i = 0; i < 11; i++) {
    ...
    int char_width = (i == 5 || i == 6 || i == 8 || i == 9 || i == 10)
                         ...
                         : 32; // "IP" 是符号，其他是汉字
    int char_height = 32;
    display_char_or_hanzi_on_lcd(lcd, fixed_ip_text_data[i], char_width,
                                 char_height, x, y_offset, scale);
    x += char_width * scale; // 移动到下一个字符位置
  }
}

void display_fixed_time_text_on_lcd(uint32_t (*lcd)[800], int x_offset,
                                    int y_offset, int scale) {
  ...
  for (int i = 0; i < 11; i++) {
    // 根据字符的实际宽度和高度显示字符
    int char_width =
        (i == 8 || i == 9 || i == 10) ? 16 : 32; // "IP" 是符号，其他是汉字
    int char_height = 32;
    display_char_or_hanzi_on_lcd(lcd, fixed_time_text_data[i], char_width,
                                 char_height, x, y_offset, scale);
    x += char_width * scale; // 移动到下一个字符位置
  }
}
...
int get_time(char *buffer, size_t buflen) {
  FILE *fp;
  const char *cmd = "date +\"%Y-%m-%d %H:%M:%S\""; // 修正命令字符串
...
int main(int argc, char *argv[]) {
  ...
  for (int lcdY = startY, bmpY = 0; lcdY < startY + infoHeader.biHeight / 2;
    ...
    for (int lcdX = startX, bmpX = 0; lcdX < startX + infoHeader.biWidth / 2;
      ...
      lcd[infoHeader.biHeight / 2 - lcdY - 1][lcdX] =
          b | (g << 8) | (r << 16) | (0 << 24);
    }
  }
  ...
  while (1) {
    ...
    // 显示固定文本
    display_fixed_ip_text_on_lcd(lcd, 0, 400, 2);

    // 显示 IP 地址
    display_string_on_lcd(lcd, ip, 550, 416, 16, 32, 1);

    // 显示固定文本
    display_fixed_time_text_on_lcd(lcd, 0, 300, 2);

    char time_str[20];
    if (get_time(time_str, sizeof(time_str)) != 0) {
    ...
    // 分割时间字符串为日期和时间
    char date_str[11];
    char time_part_str[9];
    sscanf(time_str, "%10s %8s", date_str, time_part_str);

    // 显示日期
    display_string_on_lcd(lcd, date_str, 612, 300, 16, 32, 1);

    // 显示时间
    display_string_on_lcd(lcd, time_part_str, 625, 334, 16, 32, 1);
